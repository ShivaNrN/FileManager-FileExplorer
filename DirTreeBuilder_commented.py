...
class Treenode:
  ...
  #It is called recursively when building the Tree. Easier to distinguish file from folders. Stores the file properties as a nested list --> [[p,n,s,t],[p,n,s,t]...]
  ...
  def print_tree(self):
    ...
    if self.size:
      size = (f"{self.size} bytes" if self.size < 1024 else f"{self.size // 1024} Kb")
...
An argument (the complete path must be passed as a string --> from ask_directory())
  ...
  -  the new node is created as follow: 
      ...
      - find the child associated with the current item. 
        In order to do so we use os.path.join() which joins the root path with the current item. If root = "C:/Level0/Level1" and path = "file1", the join would return "C:/Level0/Level1/file1"
...
def save_to_dict(lists,target_path):
  ...
  formatting the path. 
   re --> import re (regex), 
   search(r"a regex",the string to apply the regex) --> Scan through a string, looking for any location where this RE matches.
